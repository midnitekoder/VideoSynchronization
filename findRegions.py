import numpy as np
from syncVideos import syncVideos


def findOrientation(coor1, coor2):
	if coor1['frame1']==coor2['frame1']:
		if coor1['frame2']==coor2['frame2']-1:
			return 0
	if coor1['frame1']==coor2['frame1']-1:
		if coor1['frame2']==coor2['frame2']-1:
			return 1
		if coor1['frame2']==coor2['frame2']:
			return 2
def findRotation(prevEdgeOrientation,edgeOrientation):
	if prevEdgeOrientation==0 and edgeOrientation==0:
		return 0
	if prevEdgeOrientation==2 and edgeOrientation==0:
		return -0.5
	if prevEdgeOrientation==1 and edgeOrientation==1:
		return -1
	if prevEdgeOrientation==0 and edgeOrientation==1:
		return -0.66
	if prevEdgeOrientation==1 and edgeOrientation==0:
		return -0.33
	if prevEdgeOrientation==2 and edgeOrientation==1:
		return -1.5
	if prevEdgeOrientation==0 and edgeOrientation==2:
		return -2
	if prevEdgeOrientation==1 and edgeOrientation==2:
		return -3
	if prevEdgeOrientation==2 and edgeOrientation==2:
		return -4
...
#clockwise -1 anticlockwise +1 else 0
...
prevPair2=coordinateList[0] #second runner up
prevPair=coordinateList[1]
rotation=[]
tangent=[]
cutpoints=[]
prevEdgeOrientation=findOrientation(prevPair2,prevPair)
n=len(coordinateList)-1
print n
print prevEdgeOrientation
if prevEdgeOrientation==0:
	tangent.append(0)
if prevEdgeOrientation==1:
	tangent.append(-1)
if prevEdgeOrientation==2:
	tangent.append(-4)
for i in range(2,n):
	edgeOrientation=findOrientation(prevPair,coordinateList[i])
	prevPair=coordinateList[i]
	#print prevEdgeOrientation, edgeOrientation
	tangent.append(findRotation(prevEdgeOrientation,edgeOrientation))
	prevEdgeOrientation=edgeOrientation
print rotation
print tangent
for i in range(0,n-2):
	if tangent[i+1]-tangent[i]<0:
		rotation.append(-1)
	if tangent[i+1]-tangent[i]>0:
		rotation.append(1)
	if tangent[i+1]-tangent[i]==0:
		rotation.append(0)
#print rotation
prevRotation=rotation[0]
for i in range(0,n-3):
	if rotation[i]!=0:
		prevRotation=rotation[i]
	if prevRotation*rotation[i+1]<0:	
		dict1={}
		dict1['frame1']=coordinateList[i+2]['frame1']
		dict1['frame2']=coordinateList[i+2]['frame2']
		#dict['inclusive']=0
		cutpoints.append(dict1)

print cutpoints

syncVideos(coordinateList,cutpoints)
